...
HttpProxy.prototype.proxyRequest = function(request, response, next) {
  if (this.options.verbose) {
    console.log(util.format('request: %s %s', request.method, request.url));
  }
  var self = this;
  var backendOptions = {
  ...
  proxy_request.once('response', function (proxy_response) {
    ...
    if (self.options.verbose) {
      console.log(util.format('request: %s %s response: %s %s', request.method, request.url, proxy_response.statusCode, proxy_response.statusMessage));
    }
